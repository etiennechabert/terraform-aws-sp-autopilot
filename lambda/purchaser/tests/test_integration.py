...
def test_empty_queue():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        scenario.assert_true(
            response["statusCode"] == 200, f"Expected statusCode 200, got {response['statusCode']}"
        )
        scenario.assert_true(
        ...
        scenario.assert_true(
            not mock_sns.publish.called, "SNS publish should NOT be called for empty queue"
        )
...
def test_valid_purchase_execution():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        scenario.assert_true(
            mock_sp.create_savings_plan.called, "CreateSavingsPlan should be called"
        )
        scenario.assert_true(mock_sqs.delete_message.called, "Message should be deleted from queue")
        scenario.assert_true(mock_sns.publish.called, "Summary email should be sent")
        ...
        scenario.assert_true(
            "sp-12345678" in email_call[1]["Message"], "Email should contain Savings Plan ID"
        )
        scenario.assert_true(
...
def test_cap_enforcement():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        scenario.assert_true(
            mock_sqs.delete_message.called, "Message should still be deleted even when skipped"
        )
        scenario.assert_true(mock_sns.publish.called, "Summary email should be sent")
...
def test_api_error():
    ...
    with (
        patch("shared.handler_utils.initialize_clients") as mock_init_clients,
        ...
        # Verify
        scenario.assert_true(exception_raised, "Exception should be raised on API error")
        scenario.assert_true(mock_sns.publish.called, "Error email should be sent")

        # Verify error email content
        email_call = mock_sns.publish.call_args
        scenario.assert_true(
            "Failed" in email_call[1]["Subject"], "Email subject should contain 'Failed'"
        )
        scenario.assert_true(
            "Access denied" in email_call[1]["Message"], "Email should contain error message"
        )
        scenario.assert_true(
            "CloudWatch Logs" in email_call[1]["Message"], "Email should mention CloudWatch Logs"
        )
...
def test_database_sp_purchase():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        # Mock successful purchase
        mock_sp.create_savings_plan.return_value = {"savingsPlanId": "sp-db-12345678"}
        ...
        scenario.assert_true(
            mock_sp.create_savings_plan.called, "CreateSavingsPlan should be called for Database SP"
        )
        scenario.assert_true(mock_sqs.delete_message.called, "Message should be deleted from queue")
        scenario.assert_true(mock_sns.publish.called, "Summary email should be sent")
...
def test_database_sp_cap_enforcement():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        mock_sqs.receive_message.return_value = {
            "Messages": [
                {"Body": json.dumps(purchase_intent), "ReceiptHandle": "receipt-handle-db-456"}
            ]
        }
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        mock_sqs.receive_message.return_value = {
            "Messages": [
                ...
                {
                    "Body": json.dumps(database_intent_exceeds),
                    "ReceiptHandle": "receipt-database-2",
                },
            ]
        }
        ...
        scenario.assert_true(
            "sp-compute-111" in email_call[1]["Message"], "Email should contain Compute SP ID"
        )
        scenario.assert_true(
            "sp-database-222" in email_call[1]["Message"], "Email should contain Database SP ID"
        )
...
def test_malformed_message():
    ...
    with patch("shared.handler_utils.initialize_clients") as mock_init_clients:
        ...
        mock_sqs.receive_message.return_value = {
            "Messages": [
                {"Body": json.dumps(malformed_intent), "ReceiptHandle": "receipt-handle-malformed"}
            ]
        }
        ...
        email_call = mock_sns.publish.call_args
        scenario.assert_true(
            "Failed Purchases: 1" in email_call[1]["Message"], "Email should show 1 failed purchase"
        )
        scenario.assert_true(
            "Validation error" in email_call[1]["Message"], "Email should mention validation error"
        )
