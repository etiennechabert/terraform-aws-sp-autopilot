...
def test_all_severity_color_codes():
    ...
    for severity, expected_color in test_cases:
        result = notifications.format_slack_message("Test", ["Test message"], severity=severity)
...
def test_all_severity_emoji_indicators():
    ...
    test_cases = [("success", "✅"), ("warning", "⚠️"), ("error", "❌"), ("info", "ℹ️")]

    subject = "Test Subject"

    for severity, expected_emoji in test_cases:
        result = notifications.format_slack_message(subject, ["Test message"], severity=severity)
        ...
        assert subject in header_text, f"Subject not found in header for severity '{severity}'"
