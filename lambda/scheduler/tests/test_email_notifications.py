...
def sample_coverage():
    ...
    return {"compute": 75.5, "database": 85.0, "sagemaker": 60.0}
...
def test_send_scheduled_email_success(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_scheduled_email_message_content(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_scheduled_email_includes_queue_url(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_scheduled_email_api_error(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_scheduled_email_multiple_plans_formatting(
    mock_sns_client, mock_config, sample_coverage
):
...
def test_send_dry_run_email_message_content(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_dry_run_email_includes_enablement_instructions(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
...
def test_send_dry_run_email_api_error(
    mock_sns_client, mock_config, sample_coverage, sample_purchase_plans
):
    """Test error handling when SNS publish fails."""
    from botocore.exceptions import ClientError
...
def test_send_dry_run_email_multiple_plans_formatting(
    mock_sns_client, mock_config, sample_coverage
):
...
def test_dry_run_email_with_high_coverage(mock_sns_client, mock_config, sample_purchase_plans):
    ...
    coverage = {"compute": 99.9, "database": 98.5, "sagemaker": 97.3}
